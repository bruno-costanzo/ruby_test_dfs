...
def count_figures(matrix)
  ...
  matrix.each_with_index do |row, x|
    row.each_with_index do |_element, y|
      next unless matrix[x][y] == 1 && !visited[x][y]
...
def dfs(matrix, row, col, visited, tail = [])
  rowNbr = [0, 0, 1, -1]
  colNbr = [1, -1, 0, 0]
  ...
  4.times do |i|
    next unless valid?(matrix, row + rowNbr[i], col + colNbr[i], visited)
